...
module.exports = class FishyGame extends events {
  constructor(options = {}) {
    ...
    if (!options.balance) options.player.balance = 50;
    ...
    if (typeof options.balance !== 'number') throw new TypeError('INVALID_PLAYER: player money must be a number.');
  ...
  async catchFish() {
    ...
    if (this.balance < this.options.fishyRodPrice) return this.sendMessage({ content: this.options.noBalanceMessage });
    const content = this.options.catchMessage.replace('{fish}', fish.emoji).replace('{amount}', this.options.fishyRodPrice);


    this.balance -= this.options.fishyRodPrice;
    ...
    await coin.set(this.player.id, this.balance);
  ...
  async sellFish(type, amount) {
    ...
    this.balance += (fish.price * amount);
  ...
  async fishyInventory() {
    ...
    .addFields({ name : 'Balance', value : `**${this.balance.toLocaleString()}**`})
    .setTimestamp()
